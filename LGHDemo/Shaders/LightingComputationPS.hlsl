...
#define SHADOW_RATE 2
//#define USE_UE4_RANDOM
...
RWTexture2D<float> runningSum : register(u2);
RWTexture2D<uint> lockImage : register(u3);
RWTexture2D<uint> vplSampleBuffer : register(u4);
RWTexture2D<float> pdfBuffer : register(u5);
	...
	uint level, uint instanceId, int2 screenPos)
{
	...
	float bias = 0.01 * sceneRadius; //clamping term
	bias *= bias;
	...
	if (level >= minShadowLevel && shadowRate > 0)
	{
			...
			for (int yoff = 0; yoff < shadowRate; yoff++)
			{
				for (int xoff = 0; xoff < shadowRate; xoff++)
				{
					...
					RandomSequence_Initialize(RandSequence, 
						(shadowRate * scrWidth * screenPos.y + shadowRate * screenPos.x) + id, instanceId + frameId);
					RandSequence.Type = 0;
						...
						while (keepWaiting)
						{
							...
							if (lock == 0)
							{
								...
								{ //replace value in buffer
									vplSampleBuffer[subScreenPos] = instanceId;
		...
		else
		{
			...
			noise0 = blueNoiseTex0[screenPos & int2(127, 127)];
			noise1 = blueNoiseTex1[screenPos & int2(127, 127)];
			...
			for (int yoff = 0; yoff < shadowRate; yoff++)
			{
				for (int xoff = 0; xoff < shadowRate; xoff++)
				{
					...
					float3 randomPos;
					{
						...
						rand_1 = float3(frac(halton[id].w + noise1.x), frac(halton[id].x + noise1.y), frac(halton[id].y + noise1.z));
						if (rand_1.x > (cos(randomPos.x*PI) + 1)*0.5) randomPos.x -= 1.0;
						if (rand_1.y > (cos(randomPos.y*PI) + 1)*0.5) randomPos.y -= 1.0;
						if (rand_1.z > (cos(randomPos.z*PI) + 1)*0.5) randomPos.z -= 1.0;
					}
					...
					if (pdf > 0.0)
					{
						...
						while (keepWaiting)
						{
							...
							if (lock == 0)
							{
								...
								float2 subPos = float2(screenPos)+float2(0.5 + xoff * 1.0, 0.5 + yoff * 1.0) / shadowRate;
								//roll a dice
								float prob = hash3D(float3(subPos, float(instanceId - minSampleId)));
								if (prob < pdf / sum)
								{ //replace value in buffer
									vplSampleBuffer[subScreenPos] = instanceId;
									pdfBuffer[subScreenPos] = pdf;
