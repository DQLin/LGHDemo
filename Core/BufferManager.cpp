...
void Graphics::InitializeRenderingBuffers( uint32_t bufferWidth, uint32_t bufferHeight )
{
    ...
    esram.PushStack();
		...
		g_ScenePositionBuffer.Create(L"Position Buffer", bufferWidth, bufferHeight, 1, DXGI_FORMAT_R16G16B16A16_FLOAT, esram);
		g_SceneNormalBuffer.Create(L"Normal Buffer", bufferWidth, bufferHeight, 1, DXGI_FORMAT_R8G8B8A8_SNORM, esram);
