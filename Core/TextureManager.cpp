...
void Texture::CreateFromRawRGBA8Data(const void* data, int imageWidth, int imageHeight, bool sRGB)
{
	Create(imageWidth, imageHeight, sRGB ? DXGI_FORMAT_R8G8B8A8_UNORM_SRGB : DXGI_FORMAT_R8G8B8A8_UNORM, data);
}
...
const ManagedTexture * TextureManager::LoadFromRawData(const std::wstring & fileName, int imageWidth, int imageHeight, const void * data, bool sRGB)
{
	auto ManagedTex = FindOrLoadTexture(fileName);
	ManagedTexture* ManTex = ManagedTex.first;
	const bool RequestsLoad = ManagedTex.second;
	...
	ManTex->CreateFromRawRGBA8Data(data, imageWidth, imageHeight, sRGB);
	return ManTex;
}
